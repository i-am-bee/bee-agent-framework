...
export type RePlanState = z.output<Awaited<ReturnType<typeof createRePlanOutputSchema>>>;
              ...
              .object({
                ...
                calls: z.array(
                  z.discriminatedUnion(
                    "name",
                    await Promise.all(
                      map(tools, async (tool: AnyTool) => {
                        const schema = await tool.inputSchema();
                        if (!(schema instanceof ZodSchema)) {
                          throw new ValueError(
                            "RePlan cannot work with tools with non-zod schemas.",
                          );
                        }
...
export const RePlayAssistantPrompt = new PromptTemplate({
